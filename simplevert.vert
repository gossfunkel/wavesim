...
void main() {
	...
	vec3 charge2 = vec3(mod(sys_scale/2+(sys_scale/2* -sin(frameNum)),sys_scale),
						mod(sys_scale/2+(sys_scale/2* -cos(frameNum)),sys_scale),
						mod(sys_scale/2+(sys_scale/2* sin(frameNum)),sys_scale));*/
	vec3 charge1 = vec3(sys_scale/2+(sys_scale/2*sin(frameNum)),
						sys_scale/2+(sys_scale/2*cos(frameNum)),
						sys_scale/2+(sys_scale/2* -sin(frameNum)));
	vec3 charge2 = vec3(sys_scale/2+(sys_scale/2* -sin(frameNum)),
						sys_scale/2+(sys_scale/2* -cos(frameNum)),
						sys_scale/2+(sys_scale/2* sin(frameNum)));
	float coulForce1 = coulomb(p3d_Vertex.xyz - charge1,1.,50.);
	float coulForce2 = coulomb(p3d_Vertex.xyz - charge2,1.,25.);
	col = vec4(coulForce1+coulForce2, coulForce1/2, coulForce2/2, coulForce1+coulForce2);
